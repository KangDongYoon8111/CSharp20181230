...
class CartTest
{
    static void Main()
    {
        ...
        Console.WriteLine("CarNo  상품코드  상품명  단가  수량  합계금액");
        Console.WriteLine("-------------------------------------------------");

        int i = 0;
        foreach(KeyValuePair<int, Cart> d in carts)
        {
            Console.WriteLine($"{++i}       {d.Value.goods.goodsno}     {d.Value.goods.gname}    {d.Value.goods.danga}    {d.Value.count}    {d.Value.sum}");
        }
    }
}